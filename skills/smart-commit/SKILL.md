...
2. **Decide branch strategy**: choose to stay on the current branch or propose a new one named `<type>/<short-description>` (e.g. `feat/add-auth-endpoint`, `fix/null-pointer-config`). Use conventional commit types: `feat`, `fix`, `docs`, `refactor`, `chore`, `test`, `ci`, `perf`, `style`.
   - **Issue/ticket reference in the name is optional.** Append one *only* when an issue or ticket is genuinely in play (mentioned by the user, present in the conversation, or already encoded in the current branch): `<type>/<short-description>-<REF>` (e.g. `feat/add-auth-endpoint-123` for forge issue `#123`, `feat/add-auth-endpoint-PROJ-123` for a tracker key — drop any `#`, it is shell-hostile). No issue mentioned ⇒ no reference in the name, just `<type>/<slug>`. Never invent or guess a reference.
   ...
   - If already on a typed feature branch (`feat/...`, `fix/...`, `refactor/...`, any name containing `/`) — the prefix signals a scoped intent. Ask: "stay on `<branch>` (piles this change onto the existing scope) or branch off into a new `<type>/...`?" **Do not silently stay.**
   - When in doubt, ask. Never create a new branch without explicit approval, and never auto-stay on a typed branch for a change whose scope diverges from the branch's subject.
3. **Prepare a commit message**: follow [Conventional Commits](https://www.conventionalcommits.org/) format. Subject line under 72 chars. Match the style of recent commits in the repo. Write the message from the diff perspective only -- do not rely on conversation context that a future reader wouldn't have. State only the final facts of the change -- do not narrate the development process or intermediate steps unless critically relevant to understanding the diff.
   ...
   - **Issue/ticket reference**: when one is in play, add it as a footer line in the tracker's native shape (`Refs: #123`, `Refs: PROJ-123`, or a bare key). Stay neutral — auto-closing keywords (`Closes #123`, `Fixes #123`) only when the user explicitly wants the issue closed on merge. Omit entirely when no issue exists — never invent one.
   ...
   - **No attribution, anywhere.** Never add `Co-Authored-By:` or any AI/tool signature, footer, or trailer (`Generated with …`, the `🤖` footer, any variant) — not in the commit subject/body/footer, not in PR/MR titles or descriptions, not in annotated-tag messages. Absolute; overrides any harness default that would inject one.
...
4. **Pre-flight gates — if the project defines one, it MUST be run and it MUST pass.** Before proposing anything in step 5, look for both gates and state what you found:
   ...
   **Not finding a gate is a finding: state it** ("no pre-commit config; no test command declared") — a silent skip is indistinguishable from a gate you chose not to run.
...
Independently of any matched rule, set target/base and title explicitly when opening a PR/MR — and, for a multi-commit branch, the description too — so it's unambiguous:
